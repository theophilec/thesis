...
\begin{remark}
In the context of filtering, we are interested in the case where $f > 0$ and $f\in W^\beta_2$, thanks to the mixing hypothesis introduced in \cref{assumption:mixing}. These conditions imply \cref{assumption:target_function}. In the more general case where $f$ touches zero, an identical result can be obtained by changing ``constant'' terms in front of the rate as described in \cite{sampling-ulysse} (basically changing the norm since $\sqrt{f}$ is not in $W^\beta_2$).
\end{remark}
...
\begin{align}
    \Vert \pi_k - \hat\pi_k\Vert_{TV} ~~\leq~~ \frac{C}{\mixing^2}\left(\frac{1-\mixing^2}{1 + \mixing^2}\right)^{k-1}\|\pi_0 - \hat{\pi}_0\|_{TV} ~~+~~ \left(1 + \frac{C}{\mixing^4}\right)\frac{\varepsilon}{\sigma},
\end{align}
...
Assuming that $\hat{\pi}_0 = \pi_0$ the proposed algorithm \cref{alg:fullfilter} for any $k \in\N$ achieves an error $\|\pi_k - \hat{\pi}_k\|_{TV} \leq \left(1 + \frac{C}{\mixing^4}\right)\frac{\varepsilon}{\sigma}$ producing a Gaussian PSD model $\hat{\pi}_k$ that satisfies
