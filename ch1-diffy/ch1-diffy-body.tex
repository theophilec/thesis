...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.45\textwidth]{ch1-diffy/figures/fig_rotation/rotation}
...
\widehat{D}_\la(f,g) = \la \|\widehat{A}^* (\widehat{B} \widehat{B}^* + \la I)^{-1} \widehat{A}\|_{op}.
}
\end{lemma}
...
$$|\widehat{D}_\la(f,g) - D_\la(f,g)| \leq c\big(\tfrac{\log\frac{1}{\delta}}{\la\sqrt{N}} + \tfrac{(\log \frac{M_X}{\delta})^{\alpha}}{\la M_X^{s/d}} + \tfrac{(\log\frac{M_Y}{\delta})^{\beta}}{\la M_Y^{z/p}}\big),$$
...
The final form of the empirical estimator $\widehat{D}_\la$ is \cref{eq:widehatD-finite-dimensional}. An efficient algorithm to compute it consists in (1) first computing the matrices $\widehat{A}$ and $\widehat{B}$, then the inverse $\widehat{C} = (\widehat{B}\widehat{B}^* + \la)^{-1}$ and finally compute the largest eigenvalue of $\widehat{A}^* \widehat{C} \widehat{A}$ via a power iteration method \cite{trefethen1997numerical}. %that is a method based on matrix vector products. In particular this last step avoids to compute explicitly the product of the three matrices, $\widehat{A}^* \widehat{C} \widehat{A} v = \widehat{A}^* (\widehat{C} (\widehat{A} v))$.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.5\textwidth]{ch1-diffy/figures/fig_regularization/regularization.pdf}
