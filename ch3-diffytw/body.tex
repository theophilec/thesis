%\todo[inline]{Create two parts in manuscript: Part 1 for Diffy + DiffyTW and Part 2 for HMM}
\section{Introduction}

Time-series are ubiquitous in data analysis applications. Some common questions include: What will the weather be like tomorrow? (forecasting) Does this sequence of sensor readings come from a normal machine or a faulty machine? (classification) Among these ECG graphs from a hospital's patients, what groups exist ? (clustering)

These applications rely on pair-wise comparaisons of time series. Indeed, for forecasting, one needs to compare the forecast to the observed conditions a posteriori (or at training time) ; for classification, one can rely on a divergence-based method or embedding such as kNN ; and clustering is at its heart a distance-based method. However, time-series cannot be compared as if they were vectors in $\mathbb R^T$ where $T$ is their length. First of all, the length can be different for different samples: when comparing machines over the course of a year, one could have been measured once a month while the other could have been inspected twice a month. Second, the points are not necessarily aligned in time: all the machines in the factory are not inspected on the same day.

\paragraph{DTW family of divergences} Dynamic Time Warping (DTW) was introduced by \cite{missing} to compare time series and is the \emph{de facto} standard for comparing time series. Intuitively, Dynamic Time Warping does a matching between the points in each time series, while respecting the order of the points. The cost incurred by each matching is the euclidean distance between the points, and the DTW distance between two time series in the sum of the costs. Formally, consider $\mathcal A_{n, m}$ the set of alignment matrices, i.e. binary matrices of size $n\times m$ such that $A_{1,1}=A_{n, m}=1$ and if $A_{i,j} = 1$ then exactly one of the following is true: $A_{i+1, j}=1$ or $A_{i, j+1}=1$ or $A_{i+1, j+1}=1$. Then:
...
\begin{definition}[Soft Dynamic Time Warping]
\begin{equation}
